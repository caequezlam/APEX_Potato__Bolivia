# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 22:03:31 2021
...
WAYCHACSV=np.array([ dataway[jj][10] for jj in range(5405,5592)])
...
"OBSERVED Waycha"
Obsway1=np.array([ Observed[jj][2] for jj in range(0,187)])
...
"PBIAS - NSE- R2 ---------WAYCHA"
WAYCHAstat=np.array([ dataway[jj][10] for jj in range(5405,5592)])
...
plt.ylabel('Biomass accumulation [g/m2]',**font,color='k',size=19)
plt.xlabel("Month",**font,color='k')
plt.yticks(boundsY, boundsY,**font, color='k', size=19)
plt.annotate("Waycha - S. t. andigenum",(729677,2000),**fontitalic,color='k',size=23)
plt.annotate("PBIAS= "+ str(PBIAS_WAYCHA),(729708,550),**font,color='k',size=19)
plt.annotate("NSE= "+ str(NSE_WAYCHA),(729708,700),**font,color='k',size=19)
plt.annotate("R2= "+ str(R2way),(729708,400),**font,color='k',size=19)
...
weatherdate11=np.array([ weatherdate[jj][0] for jj in range(0,184)])
dateweather1= mdates.datestr2num(weatherdate11)
mdates.num2date(dateweather1[0])



dateweather2=np.array([ weatherdate[jj][1] for jj in range(0,184)])

dateweather3= np.array([ weatherdate[jj][2] for jj in range(0,184)])

RAIN=np.array([ dataway[jj][26] for jj in range(5405,5589)])
RAIN = RAIN.astype(float)
RAIN =np.array(RAIN)

TMAX=np.array([ dataway[jj][23] for jj in range(5405,5589)])
TMAX = TMAX.astype(float)
TMAX =np.array(TMAX)
T1=TMAX

TMIN=np.array([ dataway[jj][24] for jj in range(5405,5589)])
...
boundsX = np.linspace(0,184,8,dtype=int)
fig, ax = plt.subplots(figsize=(9,6))
...
ax.axhline(TMIN[19], color="k",linewidth=2)
